 ...
 *
 * @param {{ startsAtMs: number, recurrence: 'none' | 'daily' | 'weekly', weekdays: readonly number[], tz: string }} event
...
export function expandOccurrences(event, fromMs, toMs, maxCount = 366) {
  ...
  if (event.recurrence === 'none') {
    return event.startsAtMs >= fromMs && event.startsAtMs <= toMs ? [event.startsAtMs] : [];
  }
  ...
  // 2-day back-off absorbs any tz-offset skew in the estimate.
  const firstDay = Math.max(0, Math.floor((fromMs - event.startsAtMs) / DAY_MS) - 2);
  ...
  for (let i = 0; i <= MAX_SCAN_DAYS && out.length < maxCount; i++) {
    ...
    if (ts > toMs) break;
    if (ts < event.startsAtMs || ts < fromMs) continue;
