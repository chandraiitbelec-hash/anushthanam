/**
 * Uploads Shani Chalisa (Hindi devotional tradition, attributed to King
 * Dasharatha's original praise as refined by the poet Lalita per its own
 * closing colophon doha) to shloka_stanzas.
 * Source: the codex doc `codex/chalisa docs/shani-chalisa-multilingual.md`
 ...
 * (Closing)"), mirroring upload-lakshmi-chalisa.mjs's identical situation
 * (which itself combines upload-hanuman-chalisa.mjs's opening merge and
 * upload-shiv-chalisa.mjs's closing merge). All 44 traditional verse-lines
 * are present, spread across exactly 42 database rows.
 *
 * IAST note: candrabindu occurs in this text (11 instances) and is fixed
 ...
 * carried over from upload-hanuman-chalisa.mjs; this text is the first of
 * the three "regenerate the meanings" chalisas where it actually fires --
 * राम occurs standalone once, as part of the poet-signature phrase "राम
 * सुन्दर" (a personal name, rendered "Rām Sundara" per the same standalone-
 * word convention, not "Rāma Sundara"). Two other राम occurrences in this
 * text (रामहिं, रामचन्द्र) are compounds and are untouched, matching how
 * the fix is a token-level (not substring-level) replace.
 *
 * meaning_en/meaning_hi/meaning_te/meaning_ta are all populated from the
 * doc's per-stanza meanings, matching upload-krishna-chalisa.mjs and
 * upload-lakshmi-chalisa.mjs.
 *
 * Defaults to a dry run. Pass --write to apply.
 * Run: node scripts/upload-shani-chalisa.mjs          (dry run)
 *      node scripts/upload-shani-chalisa.mjs --write  (apply)
 */
...
const SLUG = 'shani-chalisa';
const DOC_PATH = '/Users/ChandraKanth/Documents/dev_experiments/codex/chalisa docs/shani-chalisa-multilingual.md';
