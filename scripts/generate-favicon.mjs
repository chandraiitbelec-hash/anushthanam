// One-off asset-gen script: rebuilds app/favicon.ico from the same lotus
// mark used for the PWA icons (see generate-pwa-icons.mjs) instead of the
// framework-default favicon.ico that was never replaced with the real
// brand mark. Not part of the build — run manually when the mark changes.
//
// sharp can't write .ico directly, so this renders a PNG and wraps it in a
// minimal single-image ICO container (PNG-in-ICO, supported by all modern
// browsers and Windows Vista+) by hand rather than adding a new dependency.
...
const BG = '#FAF7F2'; // --color-bg (light) — matches the PWA icons' canvas
const GOLD = '#B8860B'; // --color-gold
const SIZE = 64;
const CONTENT_FRACTION = 0.7;

// Same path data as components/Nav.tsx / generate-pwa-icons.mjs (viewBox 0 0 1018 856).
const LOTUS_PATHS = `
...
function buildSvg(size, contentFraction) {
  const MARK_W = 1018, MARK_H = 856;
  const scale = Math.min((size * contentFraction) / MARK_W, (size * contentFraction) / MARK_H);
  const markWidth = MARK_W * scale;
  const markHeight = MARK_H * scale;
...
// Minimal single-image ICO container wrapping a PNG payload (ICO format
// supports embedding PNG data directly since Windows Vista; all modern
// browsers accept this).
