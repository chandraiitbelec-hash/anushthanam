/**
 * Uploads the 108-name Radha Ashtottara Shatanamavali to shloka_stanzas.
 * Source has no ॐ prefix on each name (unlike the others) -- the shared
 * generator always prepends the fixed ॐ/ఓం/ஓம்/ōṃ literal regardless of
 * input, so this is handled automatically, matching site convention.
 *
 * Run: node scripts/upload-radha-ashtothram.mjs          (dry run)
 *      node scripts/upload-radha-ashtothram.mjs --write  (apply)
 */
...
await runUpload({ slug: 'radha-ashtothram', names, expectedCount: 108, scriptUrl: import.meta.url });
