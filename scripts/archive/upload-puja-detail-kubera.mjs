/**
 * Uploads detailed content for kubera-puja:
 ...
 *
 * Usage:
 *   node scripts/upload-puja-detail-kubera.mjs           (dry run)
 *   node scripts/upload-puja-detail-kubera.mjs --write   (apply)
 *
 * FLAGS (uncertainties to review before publishing):
 *   [FLAG-KUBERAPUJA-1] Kubera dhana prapti mantra (step 8) follows the
 *       South Indian Agamic form. North Indian tantric lineages use a
 *       different form. Consult a pandit for the correct sampradaya mantra
 *       before publishing for a North Indian audience.
 *   [FLAG-KUBERAPUJA-2] Tamil translations authored from standard liturgical
 *       Tamil — should be reviewed by a native speaker before marking
...
const content = JSON.parse(
  readFileSync(resolve(__dirname, '../research/kubera-puja-content.json'), 'utf8')
);
