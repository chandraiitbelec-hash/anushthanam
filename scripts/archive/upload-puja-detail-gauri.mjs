/**
 * Uploads detailed content for gauri-puja:
 ...
 *
 * Usage:
 *   node scripts/upload-puja-detail-gauri.mjs           (dry run)
 *   node scripts/upload-puja-detail-gauri.mjs --write   (apply)
 *
 * FLAGS (uncertainties to review before publishing):
 *   [FLAG-GAURIPUJA-1] Gauri Vrat Katha (step 8) is not yet in the
 *       stories_index tab. When the katha is added, update
 *       recite_shloka_slug for step 8 to point to the story slug.
 *   [FLAG-GAURIPUJA-2] Tamil translations authored from standard liturgical
...
const content = JSON.parse(
  readFileSync(resolve(__dirname, '../research/gauri-puja-content.json'), 'utf8')
);
