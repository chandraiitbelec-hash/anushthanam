...
/**
 * The enquiry form itself — the §9.1 demand test's one input, shared by every
 * entry point that offers it: the card at the foot of a booking-intent puja
 * page (PanditEnquiryBlock) and the standalone /find-a-pandit page.
 *
 * One component rather than two so the thing being measured is identical
 * wherever it is answered. Only `source` differs, and that difference is the
 * measurement: see EnquirySource.
 *
 * **What is visible without asking, and why.** The first pass folded all
 * thirteen fields behind a button, which is a large part of why the test was
 * too quiet to measure anything. Four are now open by default — ceremony,
 * city, when, and how to reach you — and the rest sit behind one quiet
 * disclosure. Those four are the ones the §9.1 finding is computed from: three
 * are the only required columns in the table, and `timing_window` is what
 * separates a family with a ceremony to arrange from someone looking around,
 * which is the distinction the threshold turns on. Everything else genuinely
 * is detail, and a form that opens with thirteen questions on a reference page
 * is a demand rather than an offer.
 *
 * The PRD's banned-patterns list (§7.1) binds the rest: no urgency, no
 * countdown, no "limited", no count of pandits, no claim that anyone is
 * waiting. There is no list yet and the confirmation says so in as many words.
 *
 ...
 */
export default function PanditEnquiryForm({
  placement,
  source,
  intro,
  maxWidth = '440px',
  ...
  /** Serialized EnquirySource — the entry point this form is being answered from. */
  source: string;
  /** Rendered above the fields, and replaced by the confirmation once sent. */
  intro?: ReactNode;
  maxWidth?: string;
}) {
  const { lang } = useLang();
  ...
  return (
    <>
      ...
      >
        ...
        <div>
          {/* Free text, no list, no default. The platform is not scoped to any
              city and must not prime one — where enquiries come from is part
              of what this test is measuring. */}
        ...
        <div>
          {/* Open by default, unlike the other optional fields: this is the
              one that a plain date cannot express (the ceremony is happening,
              the muhurtham is not fixed) and the one that separates real
              intent from browsing, which is what the §9.1 threshold turns on.
              Burying it would cost the measurement its sharpest column. */}
        ...
        <div>
          ...
          <input
            ...
            style={inputStyle}
          />
          <p id={`${ids}-contact-hint`} className={scripted} style={hintStyle}>
            {t.panditEnquiryContactHint}
          </p>
        </div>

        {/* Everything below is detail. One disclosure rather than a second
            form: a family that wants to say more can, and one that does not
        ...
        {!showDetails ? (
          ...
          >
            ...
            <div>
              ...
              <p className={scripted} style={hintStyle}>
        ...
        {/* Honeypot. Hidden from sight, from assistive technology and from the
            tab order, so no person can reach it; anything that fills every
            input on the page will. Not display:none, which some bots skip. */}
        ...
        <div>
          ...
          <p className={scripted} style={{ ...hintStyle, margin: '12px 0 0' }}>
